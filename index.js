import{S as A,N as w}from"./assets/vendor-BNS4lbLp.js";(function(){const t=document.createElement("link").relList;if(t&&t.supports&&t.supports("modulepreload"))return;for(const r of document.querySelectorAll('link[rel="modulepreload"]'))i(r);new MutationObserver(r=>{for(const n of r)if(n.type==="childList")for(const a of n.addedNodes)a.tagName==="LINK"&&a.rel==="modulepreload"&&i(a)}).observe(document,{childList:!0,subtree:!0});function e(r){const n={};return r.integrity&&(n.integrity=r.integrity),r.referrerPolicy&&(n.referrerPolicy=r.referrerPolicy),r.crossOrigin==="use-credentials"?n.credentials="include":r.crossOrigin==="anonymous"?n.credentials="omit":n.credentials="same-origin",n}function i(r){if(r.ep)return;r.ep=!0;const n=e(r);fetch(r.href,n)}})();const y="/RhythMix/img/rhythmix-sprite-symbol-defs.svg",b=document.querySelector(".header-logo-use");b.setAttribute("href",`${y}#icon-logo`);b.setAttribute("xlink:href",`${y}#icon-logo`);const k=document.querySelector(".btn-explore"),M=document.querySelector("#artists-section");k.addEventListener("click",()=>{M.scrollIntoView({behavior:"smooth",block:"start"})});const S="https://sound-wave.b.goit.study/api";class E{constructor(){this.initialized=!1,this.backdrop=null,this.closeBtn=null,this.loader=null,this.content=null,this.elements={}}initElements(){this.initialized||(this.backdrop=document.querySelector("[data-modal-backdrop]"),this.closeBtn=document.querySelector("[data-modal-close]"),this.loader=document.querySelector("[data-loader]"),this.content=document.querySelector(".artist-modal-content"),this.elements={image:document.querySelector("[data-artist-image]"),name:document.querySelector("#artist-modal-title"),years:document.querySelector("[data-artist-years]"),sex:document.querySelector("[data-artist-gender]"),members:document.querySelector("[data-artist-members]"),country:document.querySelector("[data-artist-country]"),genres:document.querySelector("[data-artist-genres]"),biography:document.querySelector("[data-artist-bio]"),albumsList:document.querySelector("[data-artist-albums]")},this.backdrop&&this.closeBtn?(this.init(),this.initialized=!0):console.error("Required modal elements not found!"))}init(){this.closeBtn.addEventListener("click",()=>this.close()),this.backdrop.addEventListener("click",t=>{t.target===this.backdrop&&this.close()}),document.addEventListener("keydown",t=>{t.key==="Escape"&&this.backdrop&&!this.backdrop.classList.contains("is-hidden")&&this.close()})}async open(t){if(this.initElements(),!!this.backdrop){this.backdrop.classList.remove("is-hidden"),document.body.style.overflow="hidden",this.showLoader();try{const e=await this.fetchArtistData(t);this.renderArtistData(e),this.hideLoader()}catch(e){console.error("Error loading artist data:",e),this.hideLoader(),this.showError()}}}close(){this.backdrop&&(this.backdrop.classList.add("is-hidden"),document.body.style.overflow="")}showLoader(){this.loader&&this.content&&(this.loader.style.display="flex",this.content.style.display="none")}hideLoader(){this.loader&&this.content&&(this.loader.style.display="none",this.content.style.display="block")}async fetchArtistData(t){const e=`${S}/artists/${t}/albums`,i=await fetch(e);if(!i.ok)throw new Error(`HTTP error! status: ${i.status}`);return await i.json()}renderArtistData(t){if(t)try{if(this.elements.image&&(this.elements.image.src=t.strArtistThumb||"",this.elements.image.alt=t.strArtist||"Artist"),this.elements.name&&(this.elements.name.textContent=t.strArtist||""),this.elements.years){const e=this.formatYearsActive(t.intFormedYear,t.intDiedYear);this.elements.years.textContent=e}if(this.elements.sex){const e=this.elements.sex.closest(".artist-modal-meta-item");t.strGender&&e?(e.style.display="block",this.elements.sex.textContent=t.strGender):e&&(e.style.display="none")}if(this.elements.members){const e=this.elements.members.closest(".artist-modal-meta-item");t.intMembers&&e?(e.style.display="block",this.elements.members.textContent=t.intMembers):e&&(e.style.display="none")}this.elements.country&&(this.elements.country.textContent=t.strCountry||""),this.elements.genres&&this.renderGenres(t.genres),this.elements.biography&&(this.elements.biography.textContent=t.strBiographyEN||""),this.elements.albumsList&&this.renderAlbums(t.albumsList)}catch(e){console.error("Error during rendering:",e)}}formatYearsActive(t,e){return t?`${t}-${e||"present"}`:"information missing"}renderGenres(t){if(!t||t.length===0){this.elements.genres.innerHTML="";return}const e=t.map(i=>`<span class="genre-tag">${i}</span>`).join("");this.elements.genres.innerHTML=e}renderAlbums(t){if(!t||t.length===0){this.elements.albumsList.innerHTML='<p class="no-albums">No albums available</p>';return}const e=t.map(i=>this.createAlbumHTML(i)).join("");this.elements.albumsList.innerHTML=e,this.attachYouTubeListeners()}createAlbumHTML(t){const i=(t.tracks||[]).map(r=>`
    ...
    `}formatDuration(t){if(!t)return"";const e=Math.floor(t/1e3),i=Math.floor(e/60),r=e%60;return`${i}:${r.toString().padStart(2,"0")}`}attachYouTubeListeners(){this.elements.albumsList.querySelectorAll("[data-youtube-link]").forEach(e=>{e.addEventListener("click",i=>{i.preventDefault();const r=e.dataset.youtubeLink;r&&window.open(r,"_blank","noopener,noreferrer")})})}showError(){this.content&&(this.content.style.display="block",this.content.innerHTML='<p class="error-message">Failed to load artist information. Please try again later.</p>')}}const $=new E,x="https://sound-wave.b.goit.study/api",l=8;class h{constructor(){this.artistsList=document.getElementById("artists-list"),this.loadMoreBtn=document.getElementById("load-more"),this.currentPage=1,this.isLoading=!1,this.allArtists=[],this.init()}async init(){try{await this.loadArtists(),this.renderArtists(),this.setupEventListeners()}catch{this.showError()}}setupEventListeners(){this.loadMoreBtn&&this.loadMoreBtn.addEventListener("click",()=>this.loadMore())}async loadArtists(){if(!this.isLoading){this.isLoading=!0,this.showLoadingState();try{const t=await fetch(`${x}/artists`);if(!t.ok)throw new Error(`HTTP error! status: ${t.status}`);const e=await t.json();Array.isArray(e)?this.allArtists=e:e&&Array.isArray(e.artists)?this.allArtists=e.artists:e&&typeof e=="object"?this.allArtists=Object.values(e):this.allArtists=[]}catch(t){throw t}finally{this.isLoading=!1,this.hideLoadingState()}}}renderArtists(){if(!this.artistsList)return;const t=0,e=this.currentPage*l,i=this.allArtists.slice(t,e);this.artistsList.innerHTML="",i.forEach(r=>{const n=this.createArtistCard(r);this.artistsList.appendChild(n)}),this.updateLoadMoreButton()}addMoreArtists(){if(!this.artistsList)return;const t=(this.currentPage-1)*l,e=this.currentPage*l;this.allArtists.slice(t,e).forEach(r=>{const n=this.createArtistCard(r);this.artistsList.appendChild(n)}),this.updateLoadMoreButton()}createArtistCard(t){const e=document.createElement("div");e.className="artist-card";const i=t._id||t.idArtist;e.dataset.artistId=i,e.innerHTML=`
    ...
    `;const r=e.querySelector("[data-learn-more]");return r&&r.addEventListener("click",()=>{$.open(i)}),e}truncateText(t,e){return!t||t.length<=e?t:t.substring(0,e).trim()+"..."}loadMore(){this.isLoading||(this.currentPage++,this.addMoreArtists())}updateLoadMoreButton(){if(!this.loadMoreBtn)return;this.currentPage*l<this.allArtists.length?(this.loadMoreBtn.style.display="block",this.loadMoreBtn.disabled=!1):this.loadMoreBtn.style.display="none"}showLoadingState(){this.loadMoreBtn&&(this.loadMoreBtn.disabled=!0,this.loadMoreBtn.textContent="Loading...")}hideLoadingState(){this.loadMoreBtn&&(this.loadMoreBtn.disabled=!1,this.loadMoreBtn.textContent="Load More")}showError(){this.artistsList&&(this.artistsList.innerHTML=`
        <div class="artists__error">
          <p>Failed to load artists. Please try again later.</p>
        </div>
      `),this.loadMoreBtn&&(this.loadMoreBtn.style.display="none")}}document.readyState==="loading"?document.addEventListener("DOMContentLoaded",()=>{new h}):new h;const g="/RhythMix/img/rhythmix-sprite-symbol-defs.svg",v=document.querySelector(".about-logo-use");v.setAttribute("href",`${g}#icon-logo-icon`);v.setAttribute("xlink:href",`${g}#icon-logo-icon`);const f="/RhythMix/img/feedback/icon-arrow-left.svg",m="/RhythMix/img/feedback/icon-arrow-right.svg",B="https://sound-wave.b.goit.study/api/feedbacks",C=10,o={wrapper:document.querySelector(".feedback"),swiperEl:document.querySelector(".feedback-swiper"),slidesEl:document.querySelector(".feedback-swiper .swiper-wrapper"),prevBtn:document.querySelector(".feedback-arrow--prev"),nextBtn:document.querySelector(".feedback-arrow--next"),dots:Array.from(document.querySelectorAll(".feedback-pagination .dot"))};q();T();function q(){const s=document.querySelector(".feedback-arrow--prev use"),t=document.querySelector(".feedback-arrow--next use");s&&(s.setAttribute("href",f),s.setAttribute("xlink:href",f)),t&&(t.setAttribute("href",m),t.setAttribute("xlink:href",m))}async function T(){if(!(!o.wrapper||!o.slidesEl))try{const s=await _(C);P(s),R()}catch(s){L("Failed to load feedbacks."),console.error(s)}}async function _(s){const t=new URLSearchParams({limit:String(s),page:"1"}),e=await fetch(`${B}?${t.toString()}`);if(!e.ok)throw new Error(`Feedbacks request failed: ${e.status}`);const i=await e.json(),r=(i==null?void 0:i.data)??(i==null?void 0:i.feedbacks)??i??[];return Array.isArray(r)?r.slice(0,s):[]}function P(s){if(!s.length){L("No feedbacks available.");return}o.slidesEl.innerHTML=s.map(H).join(""),d(0,s.length)}function H(s){const t=(s==null?void 0:s.name)??"Anonymous",e=(s==null?void 0:s.descr)??"",i=D(Math.round(Number((s==null?void 0:s.rating)??0)));return`
    <div class="swiper-slide feedback-card">
      ...
      <p class="feedback-text">"${u(e)}"</p>
      <p class="feedback-author">${u(t)}</p>
    </div>
  `}function I(){return Array.from({length:5}).map(()=>`
        <div class="star icon">
          <i class="star-empty"></i>
          <i class="star-half"></i>
          <i class="star-filled"></i>
        </div>
      `).join("")}function R(){if(!o.swiperEl)return;const s=new A(o.swiperEl,{modules:[w],slidesPerView:1,spaceBetween:24,navigation:{prevEl:o.prevBtn,nextEl:o.nextBtn},on:{slideChange(t){d(t.activeIndex,t.slides.length)}}});d(s.activeIndex,s.slides.length)}function d(s,t){var e,i,r,n;if(o.dots.length){if(o.dots.forEach(a=>a.classList.remove("is-active")),t<=1){(e=o.dots[0])==null||e.classList.add("is-active");return}if(s<=0){(i=o.dots[0])==null||i.classList.add("is-active");return}if(s>=t-1){(r=o.dots[2])==null||r.classList.add("is-active");return}(n=o.dots[1])==null||n.classList.add("is-active")}}function L(s){o.slidesEl.innerHTML=`
    <div class="swiper-slide feedback-card">
      <p class="feedback-text">${u(s)}</p>
    </div>
  `}function D(s){return Number.isFinite(s)?Math.min(5,Math.max(0,s)):0}function u(s){return String(s).replaceAll("&","&amp;").replaceAll("<","&lt;").replaceAll(">","&gt;").replaceAll('"',"&quot;").replaceAll("'","&#39;")}const p="/RhythMix/img/rhythmix-sprite-symbol-defs.svg",c=document.querySelector(".footer-logo-use");c&&(c.setAttribute("href",`${p}#icon-logo`),c.setAttribute("xlink:href",`${p}#icon-logo`));
